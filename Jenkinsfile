...
pipeline {
    ...
    stages {
        ...
        stage('clean'){
            steps {
                updateGitlabCommitStatus name: 'lsbl_lib_java', state: 'running'
                cleanWs()
            }
        }
        ...
        stage('git pull'){
            steps {
                ...
                checkout scmGit(
                        ...
                        extensions: [[$class: 'RelativeTargetDirectory', relativeTargetDir: 'lsbl_xsd']],
                        userRemoteConfigs: [[
                            credentialsId: '69c12235-622a-4303-8714-c5be49f65a9a',
                            url: 'git@gitlab.lemonbeat.com:sw/lsbl_xsd.git'
                        ]]
                )
            }
        }  // END stage git pull

        stage('Generate classes'){
            agent {
                docker {
    ...
    post {
        ...
        failure {
            updateGitlabCommitStatus name: 'lsbl_lib_java', state: 'failed'
            addGitLabMRComment comment: 'Well, that didn´t work obviously.'
        }
        success {
            updateGitlabCommitStatus name: 'lsbl_lib_java', state: 'success'
            addGitLabMRComment comment: 'This worked, as it should have.'
        }
        aborted {
            updateGitlabCommitStatus name: 'lsbl_lib_java', state: 'canceled'
